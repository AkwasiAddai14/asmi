...
import { format, startOfMonth, endOfMonth, startOfWeek, endOfWeek, addDays, isSameDay, parseISO, eachDayOfInterval, getDay, startOfDay  } from 'date-fns'
...
import LeButton from '@/app/dashboard/bedrijven/calender/LeButton';
...
export default function Example() {
  ...
  days.forEach((day) => {
    ...
    diensten.forEach((dienst) => {
      const shiftDate = new Date(dienst.datum).setHours(0, 0, 0, 0); // Normalize shift date to remove time
      ...
      if (dayDate === shiftDate) {
        ...
        day.events.push({
          ...
          datetime: dienst.datum.toString(),
          plekken: 0,
          aanmeldingen: 0,
          aangenomen: dienst.opdrachtnemers.length,
          reserven: 0,
          href: `/dashboard/vacture/pagina/${dienst.vacature}`,
        });
      }
    });
