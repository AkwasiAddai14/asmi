...
import Vacature, { IVacature } from "../models/vacature.model";
import Dienst, { IDienst } from "../models/dienst.model";
import Bedrijf from "../models/bedrijf.model";
import Nieuwkomer from "../models/nieuwkomer.model";
import Factuur from "../models/factuur.model";
import Sollicitatie, { ISollicitatie } from '../models/sollicitatie.model';
...
import Flexpool from '../models/flexpool.model';
 ...
 export const haalgebruikerMetId = async (clerkId: any) => {
    try {
      ...
      const freelancer = await Nieuwkomer.findOne({ clerkId });
      if (freelancer) {
        return { type: 'freelancer', data: freelancer };
      }
  ...
  export const haalVacatureMetId = async (id: string) => {
    ...
    try {
        await connectToDB();
      const vacature = await Vacature.findById(id).exec(); // Haal het volledige document op
  ...
  export const haalDienstenMetId = async (vacatureId: string) => {
    ...
    try {
      ...
      const diensten = await Dienst.find({ vacature: vacatureId }).exec(); // Haal alle diensten op die bij de vacature horen
  ...
  export const haalSollicitatiesMetId = async (vacatureId: string) => {
    ...
    try {
      ...
      const sollicitaties = await Sollicitatie.find({ vacature: vacatureId }).exec(); // Haal alle sollicitaties op die bij de vacature horen
  ...
  export const berekenBedragVanAlleDiensten = async (sollicitatieId:string) => {
    ...
    try {
      ...
      const sollicitatie = await Sollicitatie.findById(sollicitatieId);
    ...
    for (const dienstData of sollicitatie.diensten) {
      const dienst = await Dienst.findById(dienstData.dienstId);
  ...
  export const creerDienst = async (input: CreerDienstInput): Promise<IDienst | null> => {
    try {
      await connectToDB();
  
      const { vacature, datum, titel, afbeelding, werktijden, uurloon, toeslagen } = input;
      const bedrag = berekenBedrag(uurloon, datum, werktijden, toeslagen);
  
      const nieuweDienst = new Dienst({
  ...
  export const createVacature = async (input: CreateVacatureInput): Promise<IVacature> => {
    try {
      await connectToDB();
  
      // Nieuwe vacature aanmaken
      const nieuweVacature = new Vacature({
      ...
      for (let datum = beginDatum; datum <= eindDatum; datum.setDate(datum.getDate() + 1)) {
        ...
        for (const werktijd of input.tijden) {
          const nieuweDienst = await creerDienst({
            ...
            afbeelding: nieuweVacature.afbeelding,
            titel: nieuweVacature.titel,
            datum: datumString,
            werktijden: werktijd,
            uurloon: input.uurloon,
      ...
      nieuweVacature.diensten = dienstenIDs;
  ...
  createVacature(vacatureInput)
    ...
    export const haalGeplaatsteVacatures = async ({ bedrijfId }: { bedrijfId: string }) => {
        ...
        try {
          ...
          const vacatures = await Vacature.find({ _id: { $in: bedrijf.vacatures }, beschikbaar: true });
      ...
      export const haalBijbehorendeDiensten = async ({ vacatureId }: { vacatureId: string }) => {
        ...
        try {
          ...
          await connectToDB();
          const vacature = await Vacature.findById(vacatureId);
      
          if (!vacature || !vacature.diensten) {
            throw new Error(`No diensten for vacature with ID ${vacatureId} or diensten not available`);
          }
      
          const diensten = await Dienst.find({ _id: { $in: vacature.diensten }, beschikbaar: true });
      ...
      export const haalRelevanteVacatures = async (freelancerId: Types.ObjectId) => {
        try {
          ...
          if(mongoose.Types.ObjectId.isValid(freelancerId)){
            freelancer = await Nieuwkomer.findById(freelancerId);
            if (freelancer && freelancer.sollicitatie && freelancer.sollicitatie.length > 0) {
              // Fetch the related Flexpool documents
              const sollicitaties = await Sollicitatie.find({ _id: { $in: freelancer.sollicitaties } }).lean() as ISollicitatie[];
              // Extract shiftArrayIds from each shift
              console.log("alle sollicitties: ", sollicitaties)
              vacatureArrayIds = sollicitaties.map( sollicitatie => sollicitatie.vacature);
          } else {
          ...
          const allVacatureArrays = await Vacature.find({beschikbaar: true});
      ...
      export const solliciteerOpVacature = async (vacatureObject: any, freelancerObject: any, diensten: any[]): Promise<ISollicitatie> => {
        try {
          // Maak een nieuw sollicitatie object aan
          const nieuweSollicitatie = new Sollicitatie({
    ...
    if (dienst) {
      dienst.opdrachtnemers = [...dienst.opdrachtnemers.filter(opdrachtnemer => 
    ...
    export const accepteerGeselecteerdeDiensten = async ({ freelancerId, sollicitatieId, dienstenIds }: AccepteerDienstenInput) => {
      try {
        ...
        await Sollicitatie.findByIdAndDelete(sollicitatieId);  // Verwijder de sollicitatie uit de database
  ...
  try {
    ...
    export const voegFreelancerToeAanDiensten = async ({ freelancerId, sollicitatieId, dienstenIds }: AccepteerDienstenInput) => {
      try {
        ...
        const diensten = await Dienst.find({ _id: { $in: dienstenIds } });
        ...
        const freelancerData = {
          ...
          rating: freelancer.rating || 0, // Standaard 0 als er geen rating is
          geboortedatum: freelancer.geboortedatum
        };
    
        // 4️⃣ Loop door alle diensten en voeg freelancer toe (voorkom duplicaten)
        for (const dienst of diensten) {
          const bestaatAl = dienst.opdrachtnemers.some((opdrachtnemer) =>
            opdrachtnemer.freelancerId.toString() === freelancerId.toString()
          );
    
          if (!bestaatAl) {
            dienst.opdrachtnemers.push(freelancerData);
    ...
    sollicitatie.diensten = sollicitatie.diensten.filter(
      dienst => !dienstenIds.some(dienstId => dienstId.toString() === dienst.dienstId.toString())
    );

    // Als de diensten array leeg is, verwijder dan de sollicitatie
    if (sollicitatie.diensten.length === 0) {
      await Sollicitatie.findByIdAndDelete(sollicitatieId); // Verwijder de sollicitatie
    ...
    export const alleDienstenAfwijzen = async ({ sollicitatieId }: AfwijzingInput) => {
      try {
        ...
        const { freelancerId, emailadres, naam } = sollicitatie.opdrachtnemer;
    
        if (!freelancerId || !emailadres) {
        ...
        await sendRejectionEmail(emailadres, emailSubject, emailBody);
    
        console.log(`Sollicitatie ${sollicitatieId} afgewezen en email gestuurd naar ${emailadres}`);
    ...
    export const haalGeplaatsteDiensten = async ({ bedrijfId }: { bedrijfId: string }) => {
      try {
        ...
        const dienst = await Dienst.find({ _id: { $in: bedrijf.diensten }, beschikbaar: true });
...
export const accepteerSollicitatieDiensten = async ({ sollicitatieId }: AccepteerSollicitatieDienstenInput) => {
  try {
    ...
    const freelancerId = new mongoose.Types.ObjectId(sollicitatie.opdrachtnemer.freelancerId.toString());
    const dienstenIds = sollicitatie.diensten.map(dienst => new mongoose.Types.ObjectId(dienst.dienstId.toString()));
...
export const trekSollicitatieIn  = async (sollicitatieId:string, freelancerId: string) => {
   try {
    ...
    await Sollicitatie.findByIdAndDelete(sollicitatieId);
  ...
  try {
    ...
    if(!dienst){
      throw new Error("Dienst niet gevonden.");
            ...
            const begintijd = new Date(`${dienst.datum}T${dienst.werktijden.begintijd}`);
    ...
    const user = await currentUser();
    const freelancer = await Nieuwkomer.findOne({clerkId: user?.id});
    ...
    dienst.opdrachtnemers = dienst.opdrachtnemers.filter(
...
export const verwijderDienstenZonderOpdrachtnemers = async () => {
  try {
    ...
    const diensten = await Dienst.find({
    ...
    for (const dienst of diensten) {
      ...
      const dienstBegintijd = dienst.werktijden.begintijd.split(':');
    ...
    console.error('Fout bij het verwijderen van diensten:', error);
    return { success: false, message: 'Er ging iets mis bij het verwijderen van de diensten.' };
  }
};
...
export const veranderDienstenNaarVoltooidEnMaakFacturen = async () => {
  try {
    ...
    const dienstenInProgress = await Dienst.find({ status: 'in progress' });
    ...
    const freelancers = await Nieuwkomer.find({ diensten: { $ne: [] } });
    ...
    for (const freelancer of freelancers) {
      ...
      const diensten = await Dienst.find({ _id: { $in: dienstenVoorFreelancer } });

      // Bereken het totaalbedrag van de diensten
      diensten.forEach(dienst => {
        totaalbedrag += dienst.bedrag;  // Voeg het bedrag van de dienst toe
      });
...
export const veranderDienstenNaarVoltooidEnMaakFacturenVoorBedrijven = async () => {
  try {
    ...
    const bedrijven = await Bedrijf.find({ diensten: { $ne: [] } });

    // 4️⃣ Voor elk bedrijf, maak een factuur aan
    for (const bedrijf of bedrijven) {
      // Bereken het totaalbedrag voor het bedrijf
      let totaalbedrag = 0;
      const dienstenVoorBedrijf = bedrijf.diensten;
      
      // Zoek de diensten voor het bedrijf
      const diensten = await Dienst.find({ _id: { $in: dienstenVoorBedrijf } });
      ...
      bedrijf.diensten = bedrijf.diensten.filter((dienstId: any) => !dienstenVoorBedrijf.includes(dienstId));
      await bedrijf.save();
...
export async function haalVacature(id: string) {
  try {
      await connectToDB();
      const vacature = await Vacature.findById(id);
      return vacature;
  } catch (error: any) {
...
export async function haalShiftMetIdDelete(shiftId: string) {
  try {
    ...
    await connectToDB();
    const shift = await Vacature.findById(shiftId).lean() as IVacature | null;;
    console.log(shift)
    if (!shift) throw new Error('Shift not found');
    return {
      ...shift,
...
export async function reageerShift({
  ...
  try {
    ...
    const shiftArray = await Vacature.findById(shiftArrayId).populate('shifts');
    ...
    const freelancer = await Nieuwkomer.findOne({clerkId : userId});
    if (!freelancer) {
      throw new Error(`Freelancer with ID ${userId} not found`);
    }

    // Ensure that shifts is an array of ShiftType
    if (Array.isArray(shiftArray.diensten) && typeof shiftArray.diensten[0] !== 'string') {
      const populatedShifts = shiftArray.diensten as unknown as IVacature[];
...
export async function verwijderShiftArray({
  ...
  try {
    ...
    const shiftArray = await Vacature.findById(shiftArrayId);
    ...
    const shiftBegintijd = new Date(shiftArray.tijden[0].begintijd);
    // Check if the ShiftsArray should be deleted based on begintijd and aanmeldingen array
    if (forceDelete || (shiftBegintijd < currentTime && shiftArray.sollicitaties.length === 0)) {
      ...
      await Bedrijf.findByIdAndUpdate(shiftArray.opdrachtgever, {
        $pull: { shifts: { $in: shiftArray.diensten } }
      });


      // Delete all shifts in the ShiftsArray
      // await Dienst.deleteMany({ _id: { $in: Vacature.diensten } });

      // Delete the ShiftsArray
      await Vacature.findByIdAndDelete(shiftArrayId);
...
export const haalFactuurDiensten = async (id:string) => {
  try {
      ...
      if(factuur){
        const diensten = await Dienst.find({_id: {$in: factuur.diensten}})
        return diensten;
      }
    } catch (error:any) {
